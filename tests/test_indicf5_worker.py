...
def worker(tmp_path, monkeypatch):
    ...
    # Attributes on the real module rather than a stand-in in sys.modules.
    # The worker imports `plain` and `load_indicf5` from here too, and
    # replacing the whole module would take those away with it.
    runtime = importlib.import_module("colab.runtime")
    monkeypatch.setattr(runtime, "_mode", mode)
    monkeypatch.setattr(runtime, "_calls", calls)
    monkeypatch.setattr(runtime, "install_patches", lambda: None)
    monkeypatch.setattr(runtime, "reset_mode", lambda: mode.update(
...
def test_the_reference_transcript_is_normalised_before_the_model_sees_it(
    ...
    The sample avoids every word in the loanword table on purpose, so this
    test measures case and punctuation only — the fold has its own below.
    """
    bundle = build_bundle(
        tmp_path, reference_text="So LET me tell you what this DOES.")
    w = worker(bundle)

    w.synthesize_segment(w.request.segments[0])

    sent = w.model.seen[0]["ref_text"]
    assert sent == "so let me tell you what this does"
    assert "." not in sent


def test_latin_loanwords_in_the_reference_transcript_are_folded(
        tmp_path, worker):
    """
    Hindi ASR writes English loanwords in Latin — hi_dub's own reference
    transcript came back as `... कि ये project असल में ...`. That is a Latin
    island inside a Devanagari transcript, which is the script boundary
    §16e measures as the cause of the leading prefix, and the
    generated text was already folded while the reference was not.
    """
    bundle = build_bundle(
        tmp_path,
        reference_text="मैं आपको बताता हूँ कि ये project असल में करता है")
    w = worker(bundle)
...
def test_a_fully_latin_reference_transcript_is_reported(tmp_path, worker, capsys):
    """
    No automatic English-to-Devanagari transliteration exists yet (§4f), so a
    reference cut from English speech cannot get a Devanagari transcript from
    the pipeline. The worker cannot fix that and must not hide it: §16e put
    the cost at 4 prefixes in 24 clips.
    """
    bundle = build_bundle(
        tmp_path,
        reference_text="So let me tell you what this actually achieves today.")
    w = worker(bundle)

    w.synthesize_segment(w.request.segments[0])

    assert "Latin word" in capsys.readouterr().out
...
def test_an_uninstrumented_segment_raises_rather_than_returning_audio(
    ...
    import sys as _sys
    assert not importlib.import_module("colab.runtime")._calls
...
def test_identity_is_reported_as_unmeasured_rather_than_as_a_number(
        tmp_path, worker):
    """
    IndicF5 exposes no speaker encoder and the earlier baseline is out of the project, so
...
# -- the reference length, which is where the earlier baseline's default is wrong ------------
...
def test_a_reference_past_the_clipping_threshold_warns(tmp_path, worker, capsys):
    ...
    a 25s reference — which is the earlier baseline's default and was this pipeline's — hands
