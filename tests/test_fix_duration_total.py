...
Read from f5_tts/infer/utils_infer.py (~line
...
Devanagari over-allocates by about 2.15x (§5a) and fills the surplus
...
the fake in the worker's own tests deliberately does not — that one exists to prove the
...
def diag(monkeypatch):
    ...
    module = pytest.importorskip("colab.runtime")
...
def test_without_it_an_english_reference_over_allocates_as_before(diag):
    ...
    behaviour that returns — and §5a measured what it does to content.
    """
    module, utils, remote = diag
