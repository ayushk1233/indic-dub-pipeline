...
# -- the cross-script refusal has to be actionable ----------------------------


def test_a_wrong_script_reference_writes_a_template_to_fill_in(tmp_path):
    """
    Every en -> hi job hits this, because the reference transcript is built by
    transcribing the reference clip and so comes back in the source script.

    The refusal is correct — there is no automatic transliteration here, and a
    script boundary between the reference audio and its transcript is the
    largest measured cause of invented speech at the head of a clip. What it
    must not do is hand somebody a sentence and expect them to retype a
    transcript from a terminal error.
    """
    import json

    from src.stages.tts.bundle.exporter import ReferenceScriptMismatch

    runner, paths = build_runner(tmp_path, "xscript")
    write_translation(paths, [(0, 0.0, 2.0, "क" * 16)])

    reference = tmp_path / "reference.wav"
    write_wav_samples(reference, tone(3.0), SAMPLE_RATE)

    with pytest.raises(ReferenceScriptMismatch) as caught:
        runner.export_bundle(
            reference_audio=reference,
            reference_text="so let me tell you what this project actually does",
        )

    template = paths.job_dir / "reference_text.todo.json"
    ...
    # The English it heard is carried over, so the only work left is the
    # script — the words are already there to copy.
    assert "project" in payload["heard"]
    assert payload["devanagari"] == ""


def test_the_refused_export_leaves_no_bundle_behind(tmp_path):
    """A half-written bundle is something the importer finds later."""
    from src.stages.tts.bundle.exporter import ReferenceScriptMismatch

    runner, paths = build_runner(tmp_path, "xscript2")
    write_translation(paths, [(0, 0.0, 2.0, "क" * 16)])
    ...
    with pytest.raises(ReferenceScriptMismatch):
        runner.export_bundle(reference_audio=reference,
                             reference_text="plain english here")

    assert not (paths.bundle / "request").exists()
