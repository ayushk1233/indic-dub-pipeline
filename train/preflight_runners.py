...
def _train(base, data_dir, val_dir, work, store, *, route="a", gpus=2, sets=(), extra=()):
...
def run(r):
    dist.init_process_group("nccl", rank=r, world_size=2, init_method="tcp://127.0.0.1:29512")
    x = torch.ones(1 << 20, device=r); torch.cuda.synchronize(); t = time.time()
    dist.all_reduce(x); torch.cuda.synchronize()
    if r == 0: open(sys.argv[1], "w").write(str(time.time() - t))
...
def g1(**_):
    ...
    for env_extra in ({}, {"NCCL_P2P_DISABLE": "1"}):
        with tempfile.NamedTemporaryFile(suffix=".txt") as f:
            try:
                subprocess.run([sys.executable, "-c", _NCCL, f.name], env={**os.environ, **env_extra},
                               timeout=60, check=True, capture_output=True)
                seconds = float(Path(f.name).read_text())
                note = " (NCCL_P2P_DISABLE=1 needed)" if env_extra else ""
                break
...
def g7(base, route="a", **_):
    ...
    def measure(frames):
        n = max(1, min(cfg["batch"]["max_samples"], frames // FRAMES_20S))
        opt = torch.optim.AdamW(params, lr=1e-5, fused=True)
        torch.cuda.empty_cache()
        torch.cuda.reset_peak_memory_stats()
        mel = torch.randn(n, FRAMES_20S, 100, device="cuda")
        text = ["".join(letters[i % len(letters)] for i in range(k, k + 300)) for k in range(n)]
        try:
            with torch.autocast("cuda", dtype=torch.float16):
                loss, _, _ = model(mel, text=text, lens=torch.full((n,), FRAMES_20S, device="cuda"))
...
def g11(base, data, out, **_):
    ...
    leg2 = _train(base, train, val, Path(out) / "g11_leg2", store, sets=sets)
...
def g13(base, out, official, **_):
    ...
    if not merged.exists():                              # the notebook writes it first, then runs official_mel on it
