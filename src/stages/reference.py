...
REFERENCE_CEILING_S below. The history here is the earlier baseline's, because the earlier baseline is what
first exposed each fault; IndicF5 is what ships (§9). The first real
...
Sample rate: the earlier baseline loads conditioning audio at 22.05 kHz. A 16 kHz reference is
...
# the earlier baseline conditions at 22.05 kHz and generates at 24 kHz. Cutting the reference
...
# How much reference audio to aim for. the earlier baseline conditions on what it is handed,
...
# (§5d). A 25 s reference therefore hands the model 15 s of audio
# described by 25 s of transcript, which is the exact mismatch §5
...
# match (§5d), so an over-long clip arrives described by a transcript
...
#
# the earlier baseline has no such cliff: `max_ref_length` truncates conditioning cleanly and
...
def reference_seconds(model: str | None) -> float:
    ...
    Unknown models get the the earlier baseline default rather than an error: a reference of
...
def build_reference(
    ...
    Length is the reason this takes a list. the earlier baseline conditions on what it is
