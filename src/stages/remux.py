...
The length to pin to is the *video stream's*, not the container's. QuickTime
.mov files written by screen recorders carry a container duration longer than
either stream — english.mov reports 64.480s at container level against a video
stream of 64.393s, hindi.mov 73.812s against 73.693s. Pinning audio to the
container while `-c:v copy` copies the video at its own length makes the two
output streams disagree by exactly that gap, which the drift check then reports
as a sync failure in a file that is not out of sync. The video stream is what a
player renders against, so it is what the audio must match.
"""
...
def probe_video_duration(media_path: Path) -> float:
    """
    Duration of the video stream, falling back to the container.

    This is the length the output is pinned to. See the module docstring for
    why the container's own duration is the wrong answer.
    """
    streams = probe_stream_durations(media_path)

    return streams.get("video") or probe_duration(media_path)
