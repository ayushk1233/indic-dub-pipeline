...
The the earlier baseline counterpart is `the earlier baseline worker` and the two implement the same
...
**The reference transcript is mandatory.** the earlier baseline clones from audio alone;
...
— §5c measured this taking the leading prefix from 1 of 7 clips to 0
...
cannot generate English (§4), so a Latin word inside a Devanagari
...
**Duration is set per segment, not left to the model.** §5a: IndicF5
...
# 1.0 has no `reference_text` field at all. the earlier baseline can run such a bundle;
...
# §16a: a single-chunk generation is clean to 19.7 s / 240 Devanagari
...
class InstrumentationError(RuntimeError):
    ...
    clips whose length came from the byte formula (§5a).
    """
...
class IndicF5Worker:
    ...
    def prepare_reference(self) -> tuple[float, str]:
        ...
        from colab.runtime import plain
        ...
        if self.reference_seconds > REFERENCE_CLIP_THRESHOLD_S:
            print(
                ...
                "shorten the transcript to match (§5d), so the text "
    ...
    def fold_reference_text(self, text: str) -> str:
        """
        Fold Latin loanwords in the reference transcript, and say so when any
        are left.

        The generated text goes through the same table; the reference
        transcript did not, which left a Latin word sitting inside an
        otherwise Devanagari transcript — Hindi ASR writes `project` and `fit`
        in Latin, and those went to the model unchanged.

        That matters more here than in generated text. §16e measured
        the script boundary between `ref_text` and `gen_text` as the cause of
        the leading prefix: a Devanagari reference transcript took it from 4
        clips in 24 to 0. A Latin island inside the reference transcript is
        that same boundary in miniature.

        Latin that survives the fold is reported and not guessed at. For a
        reference whose transcript is English throughout, that report is the
        point — there is no automatic English-to-Devanagari transliteration in
        this repo yet (§4f), so the condition is worth naming rather
        than discovering by ear.
        """
        ...
        if left:
            print(f"  !! reference transcript still holds {len(left)} Latin "
                  f"word(s): {left[:8]}{' ...' if len(left) > 8 else ''}. The "
                  "script boundary between ref_text and gen_text is what "
                  "§16e measures as the cause of the leading prefix. "
                  "Expect it on some clips.")
    ...
    def load_model(self):
        ...
        from colab.runtime import install_patches, reset_mode

        # _mode persists across calls in a live kernel, so a previous run's
        # fix_duration is still sitting there. §13a: skipping this
    ...
    def synthesize_segment(self, segment: SynthesisSegment) -> SynthesizedSegment:
        ...
        from colab.runtime import _calls, _mode
        ...
        # fix_duration sets TOTAL frames, reference included (§5a).
        ...
        if not _calls:
            raise InstrumentationError(
                ...
                "asked for (§5a, §13a). The instrumentation is not "
        ...
        # plus the reference, and confusing the two is §5a's trap.
        ...
        return SynthesizedSegment(
            ...
            # (§17) and a fabricated number would be worse than none.
            gpt_tokens=None,
