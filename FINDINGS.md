...
**The worst case so far is not garbled audio but wrong-sounding audio, and pace does not catch it.**
§4e records an arm that beat every other arm in its run on cer, extra, missing and lead, sat at
`got/slot` 1.00, had zero bad clips on all three seeds — and came back in a Russian accent. Both
signals above assume the failure makes the audio *harder* to transcribe. This one made it easier:
evenly-dentalised English is more transcribable than Indian-accented English, so the content check
rewarded exactly the change that ruined the output. There is no threshold that fixes this. A metric
that reads accent is a different instrument, not a tighter version of this one.
...
Run 2026-09-18 on Kaggle, `probe.main()`, 70 clips. `deva_dental` maps retroflex to dental
throughout; `deva_soft` adds word-initial aspiration. Both on the `deva_ref` baseline, three seeds
each, generated by `src/text/en_to_deva.py` from the reviewed `deva_hand` text.

| arm | cer | spread | bad | by ear |
|---|---|---|---|---|
| `floor (him)` | 0.035 | — | 1/7 | him |
| `deva_ref` | 0.048 | 0.032 | 0–1 of 7 | Indian; s2 near his own |
| `deva_dental` | **0.034** | 0.027 | **0/7** | **not Indian — "Russian or Scouse"** |
| `deva_soft` | **0.037** | 0.027 | **0/7** | **not Indian — "Russian or Scouse"** |

**Both dials beat the baseline on every content number and are unusable.** `deva_dental` is the
best-scoring arm in the entire run — below the baseline, below the `floor (him)` row, zero bad clips
on all three seeds — and the speaker's description of it is a Russian or Scouse accent. Had the
table been trusted, this would have shipped.

A predicted cost was written down before the run: these arms spell the loanwords in a way IndicF5
was never trained on, so CER was expected to *rise*. It fell, on all six seeds. The prediction was
not merely wrong in size, it was wrong in sign, and the reason matters — Whisper is a fluency prior
(§3b), and evenly-dentalised English is *easier* for an ASR to parse than Indian-accented English.
The metric was measuring how transcribable the audio is, which is what it measures, and that is not
the same as how right it sounds.

**The dials worked exactly as designed. The design was wrong.** Dental stops with word-initial
aspiration is a real phonetic profile — it is roughly the Slavic one — so moving off retroflex did
not move toward his English, it moved toward somebody else's. Accent is not a one-dimensional slider
from retroflex to dental with Indian English at one end and his 6 somewhere along it. His 6 is a
point in a space this script cannot navigate on one axis, and probably cannot navigate at all.

**Orthography is refuted as an accent lever.** Not because it fails to move the accent — it moves it
a long way — but because the directions it can move are not the direction wanted. Both dials are
